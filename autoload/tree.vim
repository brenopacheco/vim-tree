...
" Local variables
" b:mappings b:hidden b:line b:col b:dir b:level
...
function! tree#open(...) abort
    ...
    let b:hidden = !g:vimtree_hidden
    if isdirectory(dir)
        call s:open(v:false, dir, level)
    endif
...
fun! tree#open_sidebar() abort
    ...
    let mappings['q']['cmd'] = 'tree#close(0)'
...
" If swap is true, replace buffer in the window by the last opened buffer
" before opening the _vimtree_ buffer.
function! tree#close(swap) abort
    call s:close(bufnr(), a:swap)
endfunction
...
function! tree#up() abort
    let b:dir = system('dirname ' . b:dir)
    let b:dir = substitute(b:dir, '\n', '', 'g')
    call tree#refresh()
    echomsg 'Tree level: ' . b:level
endfunction
...
function! tree#collapse() abort
    let b:level = b:level > 1 ? b:level - 1 : 1
    call tree#refresh()
    echo 'Tree level: ' . b:level
endfunction
...
function! tree#refresh() abort
    ...
    let b:line = pos[1]
    let b:col = pos[2]
    call s:open(bufnr(), v:false, v:false)
endfunction
...
function! tree#help() abort
    for key in keys(b:mappings)
        let desc = b:mappings[key].desc
        let cmd = b:mappings[key].cmd
...
function! s:open(bufnr, dir, level) abort
    let bufname = ''
    let bufnr = a:bufnr ? a:bufnr : -1
    if !a:bufnr
    ...
    exec bufnr . 'b'
    let b:mappings = get(b:, 'mappings', {})
    let b:hidden   = get(b:, 'hidden',   1)
    let b:line     = get(b:, 'line',     1)
    let b:col      = get(b:, 'col',      1)
    let b:dir      = get(b:, 'dir',      a:dir)
    let b:level    = get(b:, 'level',    a:level)
    setlocal modifiable
    1,$d
    call append(0, s:results())
    call setpos('.', [0, b:line, b:col, 0])
    if !s:is_tree()
...
fun! tree#reset_keys(mappings)
    ...
    let b:mappings = a:mappings
    for key in keys(b:mappings)
        exec 'noremap <silent><buffer><nowait> '
            \ . key . ' :call ' . b:mappings[key].cmd . '<CR>'
    endfor
endf
...
function! s:close(bufnr, swap) abort
    let oldbuf = s:oldbufs[bufname(a:bufnr)]
    if bufexists(oldbuf) && a:swap
        exec oldbuf . 'b'
...
function! s:results()
    let cmd    = s:cmd . ' ' . s:options . ' -L ' . b:level . ' ' . b:dir
    let ignore = ''
